...
public:
	string gethash(string userString, int hashlenght)
	{
		if (hashlenght > 3)
		{
			...
			unsigned int originalSalt = this->getControlSum(userString);
			...
			int maxSalt = this->getControlSum(userString);
			...
			this->hash += this->receivingExistCodes(originalSalt);
			this->hash += this->receivingExistCodes(originalLenghtStr);
			// Adding max salts
			this->hash += this->receivingExistCodes(maxSalt);
